...
function statusColor(status: TopicStatus): string {
  if (status === "submitted") return "bg-emerald-500/20 text-emerald-300 border-emerald-500/30";
  if (status === "recorded") return "bg-amber-500/20 text-amber-300 border-amber-500/30";
...
export function GridDashboard({
  ...
  return (
    ...
    >
        ...
        <div className="relative z-10 max-w-4xl space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-2">
            {[
              ...
              <div key={s.n} className={`${ST.bannerStep} rounded-2xl p-4 text-center border`}>
                <span className="block font-mono text-xl font-bold text-indigo-400">{s.n}</span>
      ...
      {micPermissionError && (
        <div id="mic_alert_banner" className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-200 text-xs sm:text-sm flex items-start gap-3">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <button
              ...
              className="flex items-center gap-2 bg-gradient-to-r from-amber-500 via-fuchsia-500 to-indigo-600 hover:from-amber-600 hover:via-fuchsia-600 hover:to-indigo-700 text-white font-bold px-5 py-2.5 rounded-xl shadow-lg border border-white/10 transition-all cursor-pointer text-sm"
            >
            ...
            >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className={`text-[8px] px-2 py-0.5 rounded-full border font-bold uppercase ${statusColor(status)}`}>
      ...
      {displayList.length === 0 && (
        <div id="search_empty_view" className={`text-center py-20 border border-dashed rounded-2xl ${ST.emptyBanner}`}>
          <Search className="h-8 w-8 text-indigo-500 mx-auto mb-4" />
          ...
          <button
            ...
            className="mt-6 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl px-4 py-2.5 text-xs font-bold cursor-pointer"
          >
            {t("resetFilters")}
          </button>
        </div>
